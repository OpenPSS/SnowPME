...
//  2025-07-22: OpenGL: Add and call embedded loader shutdown during ImGui_ImplGLES2_Shutdown() to facilitate multiple init/shutdown cycles in same process. (#8792)
...
//  2025-06-11: OpenGL: Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed ImGui_ImplGLES2_CreateFontsTexture() and ImGui_ImplGLES2_DestroyFontsTexture().
...
//  2024-06-28: OpenGL: ImGui_ImplGLES2_NewFrame() recreates font texture if it has been destroyed by ImGui_ImplGLES2_DestroyFontsTexture(). (#7748)
...
//  2019-09-16: OpenGL: Tweak initialization code to allow application calling ImGui_ImplGLES2_CreateFontsTexture() before the first NewFrame() call.
//  2019-05-29: OpenGL: Desktop GL only: Added support for large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag.
//  2019-04-30: OpenGL: Added support for special ImDrawCallback_ResetRenderState callback to reset render state.
//  2019-03-29: OpenGL: Not calling glBindBuffer more than necessary in the render loop.
//  2019-03-15: OpenGL: Added a GL call + comments in ImGui_ImplGLES2_Init() to detect uninitialized GL function loaders early.
...
//  2018-03-06: OpenGL: Added const char* glsl_version parameter to ImGui_ImplGLES2_Init() so user can override the GLSL version e.g. "#version 150".
...
struct ImGui_ImplGLES2_Data
{
    ...
    ImGui_ImplGLES2_Data() { memset((void*)this, 0, sizeof(*this)); }
};

// Backend data stored in io.BackendRendererUserData to allow support for multiple Dear ImGui contexts
// It is STRONGLY preferred that you use docking branch with multi-viewports (== single Dear ImGui context + multiple windows) instead of multiple Dear ImGui contexts.
static ImGui_ImplGLES2_Data* ImGui_ImplGLES2_GetBackendData()
{
    return ImGui::GetCurrentContext() ? (ImGui_ImplGLES2_Data*)ImGui::GetIO().BackendRendererUserData : nullptr;
}

// OpenGL vertex attribute state (for ES 1.0 and ES 2.0 only)
#ifndef IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
struct ImGui_ImplGLES2_VtxAttribState
{
...
bool ImGui_ImplGLES2_InitLoader();
bool ImGui_ImplGLES2_InitLoader()
{
...
// Functions
bool    ImGui_ImplGLES2_Init(const char* glsl_version)
{
    ...
    // Initialize loader
    if (!ImGui_ImplGLES2_InitLoader())
        return false;

    // Setup backend capabilities flags
    ImGui_ImplGLES2_Data* bd = IM_NEW(ImGui_ImplGLES2_Data)();
...
{
    ImGui_ImplGLES2_Data* bd = ImGui_ImplGLES2_GetBackendData();
...
{
    ImGui_ImplGLES2_Data* bd = ImGui_ImplGLES2_GetBackendData();
    IM_ASSERT(bd != nullptr && "Context or backend not initialized! Did you call ImGui_ImplGLES2_Init()?");

    ImGui_ImplGLES2_InitLoader(); // Lazily init loader if not already done for e.g. DLL boundaries.

    if (!bd->ShaderHandle)
        if (!ImGui_ImplGLES2_CreateDeviceObjects())
            IM_ASSERT(0 && "ImGui_ImplGLES2_CreateDeviceObjects() failed!");
}

static void ImGui_ImplGLES2_SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height, GLuint vertex_array_object)
{
    ImGui_ImplGLES2_Data* bd = ImGui_ImplGLES2_GetBackendData();
...
void    ImGui_ImplGLES2_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (draw_data->Textures != nullptr)
        for (ImTextureData* tex : *draw_data->Textures)
            if (tex->Status != ImTextureStatus_OK)
                ImGui_ImplGLES2_UpdateTexture(tex);
    ...
    ImGui_ImplGLES2_VtxAttribState last_vtx_attrib_state_pos; last_vtx_attrib_state_pos.GetState(bd->AttribLocationVtxPos);
    ImGui_ImplGLES2_VtxAttribState last_vtx_attrib_state_uv; last_vtx_attrib_state_uv.GetState(bd->AttribLocationVtxUV);
    ImGui_ImplGLES2_VtxAttribState last_vtx_attrib_state_color; last_vtx_attrib_state_color.GetState(bd->AttribLocationVtxColor);
#endif
#ifdef IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
...
#endif
    ImGui_ImplGLES2_SetupRenderState(draw_data, fb_width, fb_height, vertex_array_object);
...
static void ImGui_ImplGLES2_DestroyTexture(ImTextureData* tex)
{
...
void ImGui_ImplGLES2_UpdateTexture(ImTextureData* tex)
{
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        ImGui_ImplGLES2_DestroyTexture(tex);
}
...
static bool CheckShader(GLuint handle, const char* desc)
{
    ...
    if ((GLboolean)status == GL_FALSE)
        fprintf(stderr, "ERROR: ImGui_ImplGLES2_CreateDeviceObjects: failed to compile %s! With GLSL: %s\n", desc, bd->GlslVersionString);
    if (log_length > 1)
    {
        ImVector<char> buf;

...
namespace Sce::Pss::Core::Crypto {
	...
	public:
		...
		static void HmacSha256(const uint8_t key[0x20], uint8_t out[0x20], const uint8_t* in, size_t insize);
		static void HmacSha256(const uint8_t key[0x20], uint8_t out[0x20], std::vector<uint8_t>& in);

		static void Md5Sum(const uint8_t* data, const size_t datasize, uint8_t md5[0x10]);
		static void Sha256Sum(const uint8_t* data, const size_t datasize, uint8_t sha[0x20]);
		static void Sha1Sum(const uint8_t* data, const size_t datasize, uint8_t sha[0x14]);

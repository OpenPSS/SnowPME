...
void aes128_ecb_encrypt_x86(const aes128_key* ctx, uint8_t* data, size_t size)
{
    const __m128i* key = (__m128i*)ctx->key;
    for (size_t i = 0; i < size; i += 16) {
        __m128i tmp = aes128_encrypt_x86(_mm_loadu_si128((const __m128i*)data+i), key);
        _mm_storeu_si128((__m128i*)data+i, tmp);
    }
}

void aes128_ecb_decrypt_x86(const aes128_key* ctx, uint8_t* data, size_t size)
{
    const __m128i* key = (__m128i*)ctx->key;
    for (size_t i = 0; i < size; i += 16) {
        __m128i tmp = aes128_decrypt_x86(_mm_loadu_si128((const __m128i*)(data+i)), key);
        _mm_storeu_si128((__m128i*)(data+i), tmp);
    }
}
...
void aes128_cbc_encrypt_x86(const aes128_key* ctx, const uint8_t* iv, uint8_t* buffer, size_t size) {
    __m128i block;
    __m128i _iv = _mm_loadu_si128((const __m128i*)iv);
    const __m128i* key = (__m128i*)ctx->key;

    for (size_t i = 0; i < size; i += 16) {
        block = _mm_loadu_si128((const __m128i*)(buffer + i));
        block = _mm_xor_si128(block, _iv);
        block = aes128_encrypt_x86(block, key);
        _mm_storeu_si128((__m128i*)(buffer + i), block);
        _iv = block;
    }
}
...
void aes128_cbc_decrypt_x86(const aes128_key* ctx, const uint8_t* iv, uint8_t* buffer, size_t size) {
    ...
    for (size_t i = 0; i < size; i += 16) {
        block = _mm_loadu_si128((const __m128i*)(buffer + i));
        decrypted_block = aes128_decrypt_x86(block, key);
        decrypted_block = _mm_xor_si128(decrypted_block, _iv);
        _mm_storeu_si128((__m128i*)(buffer + i), decrypted_block);
        _iv = block;
    }
}
// https://github.com/mmozeiko/pkg2zip/blob/master/pkg2zip_aes.c
...
void aes128_ecb_encrypt_x86(const aes128_key* context, uint8_t* data, size_t size);
void aes128_ecb_decrypt_x86(const aes128_key* context, uint8_t* data, size_t size);
void aes128_cbc_encrypt_x86(const aes128_key* ctx, const uint8_t* iv, uint8_t* buffer, size_t size);
void aes128_cbc_decrypt_x86(const aes128_key* ctx, const uint8_t* iv, uint8_t* buffer, size_t size);
...
static void xor_blocks(uint8_t* result, const uint8_t* a, const uint8_t* b) {
    for (int i = 0; i < 16; ++i) {
        result[i] = a[i] ^ b[i];
    }
}
...
void aes128_ecb_encrypt(const aes128_key* ctx, uint8_t* data, size_t size)
{
    ...
    if (aes128_supported_x86())
    {
        aes128_ecb_encrypt_x86(ctx, data, size);
        return;
    }
#endif
    for (size_t i = 0; i < size; i += 16) {
        aes128_encrypt(ctx, data+i, data+i);
    }
}
...
void aes128_ecb_decrypt(const aes128_key* ctx, uint8_t* data, size_t size)
{
    ...
    if (aes128_supported_x86())
    {
        aes128_ecb_decrypt_x86(ctx, data, size);
        return;
    }
#endif
    for (size_t i = 0; i < size; i += 16) {
        aes128_decrypt(ctx, data+i, data+i);
    }
}
...
void aes128_cbc_encrypt(const aes128_key* ctx, const uint8_t* iv, uint8_t* buffer, size_t size)
{
    ...
    if (aes128_supported_x86())
    {
        aes128_cbc_encrypt_x86(ctx, iv, buffer, size);
        return;
    }
#endif
    uint8_t current_block[16];
    uint8_t prev_ciphertext_block[16];
    memcpy(prev_ciphertext_block, iv, 16);
    for (size_t i = 0; i < size; i += 16) {
        memcpy(current_block, buffer + i, 16);
        xor_blocks(current_block, current_block, prev_ciphertext_block);
        aes128_encrypt(ctx, current_block, current_block);
        memcpy(buffer + i, current_block, 16);
        memcpy(prev_ciphertext_block, current_block, 16);
    }
}
...
void aes128_cbc_decrypt(const aes128_key* ctx, const uint8_t* iv, uint8_t* buffer, size_t size)
{
    ...
    if (aes128_supported_x86())
    {
        aes128_cbc_decrypt_x86(ctx, iv, buffer, size);
        return;
    }
#endif
    ...
    memcpy(_iv, iv, 16);
    for (size_t i = 0; i < size; i += 16) {
        memcpy(current_ciphertext_block, buffer + i, 16);
        aes128_decrypt(ctx, current_ciphertext_block, decrypted_block);
        xor_blocks(decrypted_block, decrypted_block, _iv);
        memcpy(buffer + i, decrypted_block, 16);
        memcpy(_iv, current_ciphertext_block, 16);
    }
}

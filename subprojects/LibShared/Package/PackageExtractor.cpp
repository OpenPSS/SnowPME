...
namespace Shared::Package {
	...
	int PackageExtractor::pkgClose() {
		...
		memset(&this->pkgItem, 0, sizeof(PKG_ITEM_RECORD));
	...
	int PackageExtractor::extractCurrentItem(const char* outfile) {
		pkgSeek(this->pkgHeader.data_offset + this->pkgItem.data_offset, std::ios::beg);
		...
		do {
			...
			int readSize = (sizeof(this->workBuffer) < (this->pkgItem.data_size - totalRead)) ? sizeof(this->workBuffer) : (this->pkgItem.data_size - totalRead);
		...
		} while (totalRead < this->pkgItem.data_size);
	...
	int PackageExtractor::readItemByIndex(int index, std::string& decryptedFilename) {
		// read item record entry
		this->pkgReadOffset(((this->pkgHeader.data_offset + this->pkgMetadata.item_table_offset) + (index * sizeof(PKG_ITEM_RECORD))), &this->pkgItem, sizeof(PKG_ITEM_RECORD));

		this->pkgItem.flags = __builtin_bswap32(this->pkgItem.flags);
		this->pkgItem.filename_offset = __builtin_bswap32(this->pkgItem.filename_offset);
		this->pkgItem.filename_size = __builtin_bswap32(this->pkgItem.filename_size);
		this->pkgItem.data_offset = __builtin_bswap64(this->pkgItem.data_offset);
		this->pkgItem.data_size = __builtin_bswap64(this->pkgItem.data_size);


		// read and item filename
		if(IS_PSP_CONTENT_TYPE(this->pkgMetadata.content_type) && IS_PS3_KEY(this->pkgItem.flags)) return PKG_ERROR_UNKNOWN_KEY;

		std::vector<char> buffer = std::vector<char>(this->pkgItem.filename_size + 1);
		CHECK_ERROR(this->pkgReadOffset((this->pkgHeader.data_offset + this->pkgItem.filename_offset), buffer.data(), this->pkgItem.filename_size));

		decryptedFilename = std::string(buffer.data());
		return PKG_ERROR_NO_ERROR;
	}
	...
	int PackageExtractor::ExpandPackage(const char* pkg_file, const char* output_folder, void (*progress_callback)(const char*, uint64_t, uint64_t)) {
		...
		for (int current_item = 0; current_item < this->pkgHeader.item_count; current_item++)
		{

			// update progress in GUI
			if (progress_callback != nullptr) progress_callback(pkg_file, (uint64_t)current_item, (uint64_t)this->pkgHeader.item_count);

			std::string decryptedFilename;
			if (this->readItemByIndex(current_item, decryptedFilename) != PKG_ERROR_NO_ERROR) continue;

			// create directory that this file/directory is located within;
			std::string outfile = std::filesystem::path(output_folder).append(decryptedFilename).string();
			std::filesystem::create_directories(std::filesystem::path(outfile).parent_path().string());

			switch (this->pkgItem.flags & 0x1F) {
			case PKG_TYPE_FILE:
			case PKG_TYPE_SELF:
			...
			default:
				CHECK_ERROR(this->extractCurrentItem(outfile.c_str()));
				break;
			case PKG_TYPE_DIR:

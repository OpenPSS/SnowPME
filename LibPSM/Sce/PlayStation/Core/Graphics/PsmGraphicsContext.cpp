...
namespace Sce::PlayStation::Core::Graphics {
	...
	int PsmGraphicsContext::BindArrayBuffer(uint32_t buffer) {
		GraphicsContext* context = GetContext();
		context->BoundArrayBuffer = buffer;
		glBindBuffer(GL_ARRAY_BUFFER, context->BoundArrayBuffer);
		return context->BoundArrayBuffer;
	}
